...
class OdeintAdjointMethod(torch.autograd.Function):
    ...
    def forward(ctx, shapes, func, y0, t, rtol, atol, method, options, adjoint_rtol, adjoint_atol, adjoint_method,
        ...
        with torch.no_grad():
            ...
            y = ans
            y_dt = ans_dt
            ctx.save_for_backward(t, y, y_dt, *adjoint_params)
    ...
    def backward(ctx, *grad_y):
        with torch.no_grad():
            ...
            t, y, y_dt, *adjoint_params = ctx.saved_tensors
            grad_ydt = grad_y[1]
            grad_y = grad_y[0]

            adjoint_params = tuple(adjoint_params)
            # Use this time to keep track of the Lx and Lu derivatives for PMP
            time_iter = len(t) - 1
            ...
            size_up_to_prams = len(aug_state)
            ...
            def augmented_dynamics(t, y_aug):
                ...
                dldt, y, adj_y = y_aug[:size_up_to_prams]
                ...
                with torch.enable_grad():
                    ...
                    dydt = y_dt[time_iter].detach().requires_grad_(True)
                    ...
                    dldparams = list(torch.autograd.grad(
                        func_eval, adjoint_params, grad_ydt[time_iter], allow_unused=True, retain_graph=True
                    ))
                ...
                #-dfdx - dldx
                vjp_y = vjp_y - grad_y[time_iter]

                # dfdu - Lu
                vjp_params = [torch.zeros_like(param) if vjp_param is None else vjp_param
                              for param, vjp_param in zip(adjoint_params, vjp_params)]

                dldparams = [torch.zeros_like(param) if dldparams is None else -dldparams
                             for param, dldparams in zip(adjoint_params, dldparams)]

                vjp_params = [sum(pair) for pair in zip(vjp_params, dldparams)]
            ...
            for i in range(len(t) - 1, 0, -1):
                ...
                aug_state, _ = odeint(
                    ...
                    t[i - 1:i + 1].flip(0),
                    rtol=adjoint_rtol, atol=adjoint_atol, method=adjoint_method, options=adjoint_options, backward=True
                )
            ...
            adj_y = aug_state[2]
            adj_params = aug_state[size_up_to_prams:]
...
def odeint_adjoint(func, y0, t, *, rtol=1e-7, atol=1e-9, method=None, options=None,
    ...
    shapes, func, y0, t, rtol, atol, method, options, decreasing_time = _check_inputs(func, y0, t, rtol, atol, method, options, SOLVERS)

    # Handle the adjoint norm function.
    state_norm = options["norm"]
    handle_adjoint_norm_(adjoint_options, shapes, state_norm)

    ans, ans_dt = OdeintAdjointMethod.apply(shapes, func, y0, t, rtol, atol, method, options, adjoint_rtol, adjoint_atol,
